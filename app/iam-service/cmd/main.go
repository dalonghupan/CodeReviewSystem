// iam-service 身份权限租户服务入口（HLD §2.6 / LLD §3.1）
package main
...
import (
	...
	"cr-system/app/iam-service/internal/service"
...
var configPath = flag.String("conf", "configs/iam-service.yaml", "配置文件路径")
...
func main() {
	...
	authMW, err := middleware.JWTAuth(middleware.AuthConfig{
		JWKSURL:   bc.Auth.JWKSURL,
		Issuer:    bc.Auth.Issuer,
		Audience:  bc.Auth.Audience,
		Whitelist: bc.Auth.Whitelist,
	})
	if err != nil {
	...
	v1.RegisterIAMServiceServer(grpcSrv, svc)
	...
	v1.RegisterIAMServiceHTTPServer(httpSrv, svc)
